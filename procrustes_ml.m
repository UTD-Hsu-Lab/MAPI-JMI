function [d, Z, transform] = procrustes_ml(X, Y, varargin)
%PROCRUSTES Procrustes Analysis
%   D = PROCRUSTES(X, Y) determines linear transformation (translation,
...
pnames = {'rotation'   'scaling'  'reflection'};
dflts =  {  true        true         'best'};
[doRotation,doScaling,doReflection] = internal.stats.parseArgs(pnames, dflts, varargin{:});

if ~isscalar(doRotation) || ~(islogical(doRotation) || isnumeric(doRotation))
    error(message('stats:procrustes:BadRotation'));
end
...
if ~constX && ~constY
    ...
    A = X0' * Y0;
    if doRotation
    % T is the optimum rotation matrix of Y.
    [L, D, M] = svd(A);
    T = M * L';
